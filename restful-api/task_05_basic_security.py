...
@auth.login_required
def basic_protected():
    """Returns message if basic auth credentials are valid"""
    return "Basic Auth: Access Granted"


@app.route("/login", methods=["POST"])
def login():
    """Accepts credentials and returns JWT token"""
...
@jwt_required()
def jwt_protected():
    """Returns message if JWT token is valid"""
...
@jwt_required()
def admin_only():
    """Returns message if user has admin role"""
